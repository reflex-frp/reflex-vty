...
scrolling = col $ do
  ...
  grout (fixed 1) $ text $ ffor (_scrollableText_scrollPosition out) $ \p -> "Scrolled to " <> case p of
    ScrollPos_Top -> "top"
  ...
  tile (fixed 10) $ col $ do
    ...
    ScrollableText pos total h <- tile flex $
      scrollableText (ScrollableTextConfig never never ScrollPos_Bottom (pure $ Just ScrollToBottom_Maintain)) $
        T.unlines <$> xs
